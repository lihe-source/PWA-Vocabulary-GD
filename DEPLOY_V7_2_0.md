...
本次練習天數功能不依賴 Cloudflare Worker，因此只上傳 GitHub Pages 檔案即可使用，現有提醒也會繼續運作。若希望 Worker 根網址同步顯示 `V7.2.0`，可在原本可執行 Wrangler 的終端機進入專案根目錄後執行：
...
正常情況下 Service Worker 會自動切換到 `Voc-PWA-V7_2_0`。若仍顯示舊版本：
...
- 設定頁版本顯示 `V7.2.0`、資料儲存顯示 `IndexedDB V8`。
- 主頁「今日例句」上方可看到累積練習天數卡。
